...
<div class="wrap">
  ...
  <ul class="subsubsub">
    <?php foreach ($sub as $i => $text) : ?>
      <li class="<?php echo $i ?>">
        <a href="<?php echo admin_url('options-general.php?page=admin-column-custom&type=' . $i); ?>" <?php echo $type == $i ? 'class="current"' : ''; ?>><?php echo $text ?></a>
      </li>
    <?php endforeach; ?>
  </ul>
  <form action="options.php" id="custom-columns-form" method="post">
        ...
        ?>
          <tr class="hentry-<?php esc_attr_e($column['default'] == 1 ? 'default' : 'drag'); ?>" data-name="<?php esc_attr_e($column['default'] == 1 ? $column['name'] : ''); ?>">
            ...
            <td class="column-title column-primary">
              ...
              <?php endif; ?>
              <button type="button" class="toggle-row"><span class="screen-reader-text"><?php _e('Show more details', 'admin-column-custom'); ?></span></button>
            </td>
    ...
    <div class="tablenav bottom">
      <div class="alignleft actions bulkactions">
        <input type="submit" name="submit" class="button button-primary" value="<?php _e('Save Changes', 'admin-column-custom'); ?>">
      </div>
      <div class="alignright actions bulkactions">
        <button type="button" class="button button-primary btn-add"><?php _e('Add Row', 'admin-column-custom'); ?></span>
      </div>
      <br class="clear">
    </div>
  </form>
