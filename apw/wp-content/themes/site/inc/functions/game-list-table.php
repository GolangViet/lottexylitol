 ...
 */
class Site_Game_List_Table extends WP_List_Table
{
    /** ************************************************************************
    ...
    function __construct()
    {
        ...
        parent::__construct(array(
            ...
            'plural'    => 'games',    //plural name of the listed records
    ...
    function column_default($item = [], $column_name = '')
    {
        switch ($column_name) {
            ...
            case 'score':
                // score: 88
                parse_str(str_replace([': ', ':'], '=', $item['description']), $params);

                return isset($params['score']) ? $params['score'] : '';
            default:
    ...
    function get_current_user_screen_meta($key, $default)
    {
        ...
        $v = (string) get_user_meta($current_user->ID, 'screen_meta_usergame_' . $key, $single = true);
        if ($v && $v != '') {
            if (is_array($default)) {
                $v = json_decode($v);
                if (is_object($v)) {
    ...
    function update_current_user_screen_meta($key,  $value)
    {
        ...
        return update_user_meta($current_user->ID, 'screen_meta_usergame_' . $key, json_encode($value));
    }

    function extra_tablenav($which)
    {
        if($which == 'top') {
            echo '<label>'. __('From', 'site'). '</label><input type="date" name="game_export_from" /> ';
            echo '<label>'. __('To', 'site'). '</label><input type="date" name="game_export_to" /> ';
            echo '<button type="submit" name="game_export" class="button action" value="csv">'. __('Export CSV', 'site'). '</button>';
        }
    }
    ...
    function export_csv()
    {
        ...
        $date_from  = isset($_data['game_export_from']) ? sanitize_text_field($_data['game_export_from']) : '';
        $date_to    = isset($_data['game_export_to']) ? sanitize_text_field($_data['game_export_to']) : '';

        global $wpdb;

        $table = $wpdb->prefix . 'userhistory';

        $query = "SELECT * FROM $table WHERE `name`='game' ";
        ...
        if ($items && count($items) > 0) {
            foreach ($items as $item) {
                ...
                $columns['name']    = get_the_author_meta('display_name', $item['user_id']);

                // score: 88
                parse_str(str_replace([': ', ':'], '=', $item['description']), $params);
                        
                $columns['score']   = isset($params['score']) ? $params['score'] : '';

                $columns['created'] = $item['created'];
 ...
 *
 */
function site_game_list_page_admin_init()
{
    $game_export = isset($_GET['game_export']) ? sanitize_text_field($_GET['game_export']) : '';
    if ($game_export == 'csv') {
        $site_game_table = new Site_Game_List_Table();

        $site_game_table->export_csv();
    }
}
add_action('admin_init', 'site_game_list_page_admin_init');
 ...
 *
 */
function site_game_render_list_page()
{
    ...
    if ($id > 0 && $action == 'detail') :
        ...
        $item = $site_game_table->get_item($id);
?>
        <h1><?php echo $title ?></h1>
    <?php
        site_game_render_detail($item);

    else :
        //Fetch, prepare, sort, and filter our data...
        $site_game_table->prepare_items();
    ?>
        <h1><?php echo $title ?></h1>
        <?php add_thickbox(); ?>
        ...
        <div class="wrap-list-table">
            ...
            <form id="answers-filter" method="get">
                ...
                <?php // $site_game_table->search_box('Search', 'name'); 
                ?>
                <?php $site_game_table->display() ?>
            </form>
        </div>
    <?php
    endif;
 ...
 * @param object $item 
 */
function site_game_render_detail($item)
{
    $page = sanitize_text_field(isset($_REQUEST['page']) ? $_REQUEST['page'] : '');
    ...
    <div class="site_game_detail">
        <table class="form-table" role="presentation">
            ...
            <tr>
                <th scope="row"><label>Point</label></th>
                <td><?php esc_attr_e($data) ?></td>
            </tr>
            <tr>
